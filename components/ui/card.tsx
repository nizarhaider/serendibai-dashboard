...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        "group/card flex flex-col gap-(--card-spacing) overflow-hidden rounded-[18px] border border-white/80 bg-card py-(--card-spacing) text-sm text-card-foreground shadow-[0_18px_55px_-38px_rgba(15,32,28,0.34)] ring-1 ring-border/65 backdrop-blur-sm [--card-spacing:--spacing(4)] sm:rounded-[20px] sm:[--card-spacing:--spacing(5)] has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 data-[size=sm]:[--card-spacing:--spacing(3)] data-[size=sm]:has-data-[slot=card-footer]:pb-0 *:[img:first-child]:rounded-t-[20px] *:[img:last-child]:rounded-b-[20px]",
        className
      )}
      {...props}
    />
